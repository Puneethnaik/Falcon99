...
class FalconServer:
    ...
    async def create_game_manager(self, name):
        ...
        game_manager = GameManager(id=len(self.game_managers), start_time='')
        ...
        print("The id of the game manager created is %d" % game_manager.id)
        ...
        if server_conn is None:
            ...
            print("Writing %s to worker process" % (json.dumps(game_manager.__dict__).encode('UTF-8')))
            server_conn.write((json.dumps(game_manager.__dict__) + '\n').encode('UTF-8'))
